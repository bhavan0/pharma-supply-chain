...
export class AddOrderComponent implements OnInit {
  ...
  async getMedicineInfo() {
    const blockMedicine = await this.contractService.getMedicineByIdOfDistributor(this.selectedMedicine.medicineId, this.selectedDistributor.address);
    this.maxQuantity = blockMedicine.quantity;
    this.singleUnitPrice = blockMedicine.price;
  }

  async placeOrder() {
    await this.contractService.createRetailerOrder(this.selectedDistributor.address, this.selectedMedicine.medicineId, this.orderId, this.selectedQuantity);
