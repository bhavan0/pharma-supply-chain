...
export class AddOrderComponent implements OnInit {
  ...
  async getMedicineInfo() {
    const blockMedicine = await this.contractService.getMedicineByIdOfDistributor(this.selectedMedicine.id, this.selectedDistributor.address);
    this.maxQuantity = blockMedicine.quantity;
    this.singleUnitPrice = blockMedicine.price / 10000;
  }

  async placeOrder() {
    await this.contractService.createRetailerOrder(this.selectedDistributor.address, this.selectedMedicine.id, this.orderId, this.selectedQuantity);
